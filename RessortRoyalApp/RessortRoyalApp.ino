...
#include "RessortRoyal.h"

#define FORMAT_LITTLEFS_IF_FAILED true

// #define CONFIG_ASYNC_TCP_USE_WDT 0

#define MAIN_VERSION "Version 3.1"
  ...
  if (!bu->isPlcBroken()){
    ...
    if (!bu->clientQueueIsFull() && !bu->isWebSocketMessagesListMoreThanHalf()){ // C'est la queue du serveur et la queue des message récupéré
        if (bu->isAllInputsReadOnce()){
            if (!ressortRoyal->logiqueExecutor()){
    ...
    // Queues ok
      ...
      Serial.println("Queues are full, will not call logic executor");
    }
