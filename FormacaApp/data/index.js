   ...
   class Socket {
   	...
   	watchdog() {
   		...
   		if (this.state === CONNECTING || this.state === RECONNECT){
   			...
   			// Thierry: disconnect socket before try to reconnect..
   			this.disconnect();
   			setTimeout(()=> {
   	...
   	onMessage(msg){
   		...
   		if (this.cue.length > 0){
   			for (let i = 0; i < this.cue.length; i++){
   				setTimeout(()=>this.sendMessage(this.cue[i].key, this.cue[i].msg), i*5);
   			}
   			this.cue = [];
   		}
   	...
   	onOpen(event) {
   		...
   		for (let key in this.targets){
   			// Call the listenere to enable it
   	...
   	onClose(event) {
   		...
   		if (event.code == 1000){
   			...
   			window.error("Websocket closed abnormally with code "+ event.code );
   		}
   	}
...
`;
   ...
   class HwSection extends HTMLElement {
       ...
       connectedCallback() {
           console.log("HwSection connected");
           this.render();
       ...
       updateVisibility() {
           ...
           if (card) {
               ...
               console.log(`Card visibility toggled: ${this._isVisible}`);
           } else {
       ...
       render() {
           ...
           this.updateVisibility();
           console.log(`Component rendered: ${this._descriptor.name}`);
       }
   }
...
`;
   ...
   class NoShow extends ConnectedElement {
     ...
     render(){
         ...
         else if (this._descriptor.variant === "with-children"){
             this.children.map((child) => {
                 $template.querySelector(".children").appendChild(child);
             });
         }
