...
uint32_t ModbusRTUTemplate::calculateMinimumInterFrameTime(uint32_t baud, uint8_t char_bits) {
	...
	if (baud > 19200) {
		...
		Serial.printf("calculateMinimumInterFrameTime: %u\r\n",  3.5 * charSendTime(baud, char_bits)); // Thierry
#endif
...
void ModbusRTUTemplate::setInterFrameTime(uint32_t t_us) {
	...
	Serial.printf("InterframeTime: %u [us]\r\n", t_us);
    _t = t_us;
}
...
void ModbusRTUTemplate::task() {
...
#if defined(MODBUSRTU_DEBUG)
	Serial.printf("Received %u char : %u\r\n", _port->available(), millis());
#endif	
	...
	if (isMaster) {
    	...
    	while (micros() - t < _t) { // Wait data whitespace
			...
			Serial.println("Task timeout !!!");  // Thierry 
				return;
			}
		}
	}
...
bool ModbusRTUTemplate::cleanup() {
	...
	if (_slaveId && (micros() - _timestamp > MODBUSRTU_TIMEOUT_US || (crcError && micros() - _timestamp > 100000))) { // 100 ms
		if (_cb) {
